...
ModalWindow.prototype.centralize = function () {
  var left =
    (document.documentElement.clientWidth - this.window.offsetWidth) / 2;

  var top =
    (document.documentElement.clientHeight - this.window.offsetHeight) / 2;
...
ModalWindow.prototype.setContent = function (
  content,
  centralizeMovable,
  keepMovablesRation
) {
  ...
  if (!this.movable || (this.movable && centralizeMovable)) {
    ...
    if (keepMovablesRation) {
      ...
      var percentInLeft =
        ((this.cursorX - this.window.offsetLeft) / this.window.clientWidth) *
        100;

      var percentInTop =
        ((this.cursorY - this.window.offsetTop) / this.window.clientHeight) *
        100;
      ...
      this.window.style.left =
        this.cursorX - (this.window.clientWidth / 100) * percentInLeft + "px";

      this.window.style.top =
        this.cursorY - (this.window.clientHeight / 100) * percentInTop + "px";
    } else {
